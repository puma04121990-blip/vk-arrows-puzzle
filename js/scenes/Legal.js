class LegalScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(width / 2, wide ? 58 : 78, 'Документы игры «Пульс стрелок»', {
      fontFamily: 'Arial',
      fontSize: '14px',
      color: '#6a6a82'
    }).setOrigin(0.5);
    ...
    items.forEach((item) => {
      ...
      this.add.text(width / 2 - cardW / 2 + 20, y - 12, item.title, {
        fontFamily: 'Arial Black, Arial',
        fontSize: wide ? '16px' : '17px',
        color: '#e0e0f0'
      }).setOrigin(0, 0.5);

      this.add.text(width / 2 - cardW / 2 + 20, y + 14, item.desc, {
        fontFamily: 'Arial',
        fontSize: '13px',
        color: '#6a6a82'
      }).setOrigin(0, 0.5);
    ...
    this.add.text(width / 2, y + 10, 'Начиная игру, вы принимаете условия\nсоглашения и политики конфиденциальности.', {
      fontFamily: 'Arial',
      fontSize: '13px',
      color: '#505068',
      align: 'center'
  ...
  openDoc(url) {
    // Абсолютный URL для документов на GitHub Pages
    let full = url;
